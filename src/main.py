...
if __name__ == '__main__':
    ...
    if False:
        # read prima nota
        path2file = r'..\data\UP DOWN COFFEE SRL\FATTURE XML\all.xlsx'
        df_sack, df_iva = read_Accountingxlsx_to_df(path2file)

    else:
        # import prima nota
        df_sack = pd.read_pickle('../result/df_row.pkl')
        df_iva = pd.read_pickle('../result/df_iva.pkl')


    if False:
        # read fatture
        rootdir = Path(r'..\data\UP DOWN COFFEE SRL\FATTURE XML\16.03.2020-31.12.2020\Fatture in Cloud')
        ...
        df.to_pickle('../result/df.pkl')
        df.to_excel('../result/fatture.xlsx')
    else:
        # import fatture
        df = pd.read_pickle('../result/df.pkl')
    ...
    if False:
        ...
        df.to_pickle('../result/df_row_with_sack_index.pkl')
        print("righe in prima nota",df_sack.shape)
        print("righe fatture", df.shape)

    if False:
        df_row_with_sack_index = pd.read_pickle('../result/df_row_with_sack_index.pkl')
        df_final = pd.merge(df_row_with_sack_index,df_sack,how="left", left_on="join_idx",right_index=True)
        df_final.to_pickle('../result/df_final.pkl')
        df_final.to_excel('../result/df_final.xlsx')
